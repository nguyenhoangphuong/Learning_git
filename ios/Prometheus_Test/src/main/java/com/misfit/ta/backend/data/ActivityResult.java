...
public class ActivityResult extends BaseResult 
{
	 ...
	 * searchActivities
	 * @param response the response from searchActivities
	 * @param index activity index on the list within response 
	 * @return ActivityResult
	 * @throws JSONException
	 */
	public static ArrayList<ActivityResult> getActivityResults(
			ServiceResponse response) throws JSONException
	{
		...
		for (int i = 0; i < jsonActivities.length(); i++) {
			ActivityResult activity = ActivityResult.getFields(
					jsonActivities.getJSONObject(i),
					response);
	...
	protected static ActivityResult getFields(JSONObject jsonActivity,
			ServiceResponse response) throws JSONException {
