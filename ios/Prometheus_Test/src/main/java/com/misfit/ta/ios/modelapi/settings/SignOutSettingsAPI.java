...
public class SignOutSettingsAPI extends ModelAPI {
	...
	public void e_SignUp() {
		// sign up an account
		PrometheusHelper.signUp(email, password, isMale, date, month, year,
				isUSUnit, h1, h2, w1, w2, 1);
	}
	...
	public void e_InputActivity() {
		String[] times = { String.format("%d", hour > 12 ? 12 - hour : hour),
				"00", hour < 12 ? "AM" : "PM" };
		lastStartTime = String.format("%d", hour > 12 ? 12 - hour : hour)
				+ ":00" + (hour < 12 ? " am" : " pm");
	...
	public void v_StartScreen() {
		Assert.assertTrue(ViewUtils.isExistedView("UIButtonLabel", " SIGN UP"),
				"Sign up button is avaible");
		Assert.assertTrue(ViewUtils.isExistedView("UIButtonLabel", " SIGN IN"),
				"Sign in button is avaible");
	}
	...
	public void v_ProfileSettingsAfterLogin() {
		...
		boolean male = Gui.getProperty("UIButton", "Male", "isSelected")
				.equals("1") ? true : false;
		String birthday = PrometheusHelper.formatBirthday(year + "",
				PrometheusHelper.getMonthString(month, true), date + "");
		String weight = w1 + w2 + (isUSUnit ? " lbs" : " kg");
		String height = h1 + h2 + (isUSUnit ? "" : " m");

		Assert.assertTrue(isMale == male, "Gender should be "
				+ (isMale ? "Male" : "Female"));
		Assert.assertTrue(ViewUtils.isExistedView("UILabel", birthday),
	...
	public void v_GoalSettings() {
		Assert.assertTrue(HomeSettings.isAtEditGoal(),
				"Current view is Settings - Adjust Goal");
	}
