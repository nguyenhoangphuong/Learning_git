...
test("Comment C155", function(target, app) {
     ...
     target.frontMostApp().navigationBar().leftButton().tap();
     target.frontMostApp().alert().tapWithOptions({tapOffset:{x:0.52, y:0.18}});
	 target.frontMostApp().alert().staticTexts()["Cannot connect to the server!"].tapWithOptions({tapOffset:{x:0.45, y:0.81}});
