...
target.frontMostApp().mainWindow().scrollViews()[0].dragInsideWithOptions({startOffset:{x:0.98, y:0.40}, endOffset:{x:0.11, y:0.43}});
	 ...
	 target.frontMostApp().mainWindow().scrollViews()[0].textFields()[0].setValue(email);
	 
	 target.frontMostApp().mainWindow().scrollViews()[0].textViews()[0].tapWithOptions({tapOffset:{x:0.20, y:1.76}});
	 
	 
	  target.frontMostApp().mainWindow().scrollViews()[0].textViews()[0].setValue("tatabim");
    ...
    target.delay(2);
	 ...
	 isAlert?UIALogger.logPass("C102 pass"):UIALogger.logFail("C102 fail");
