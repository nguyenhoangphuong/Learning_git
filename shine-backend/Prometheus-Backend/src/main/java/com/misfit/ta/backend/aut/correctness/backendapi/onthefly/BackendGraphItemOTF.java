...
public class BackendGraphItemOTF extends BackendAutomation {
	...
	@Test(groups = { "ios", "Prometheus", "MVPBackend", "api", "OTF", "graph_items" })
	public void CreateGraphItemsInvalidParams() {
		...
		BaseParams params = new BaseParams();
		params.addHeader("auth_token", token);
		params.addParam("graph_items", "This is not an array");
		...
		params.addHeader("auth_token", token);
		params.addParam("graph_item", "This is not an object");
	...
	@Test(groups = { "ios", "Prometheus", "MVPBackend", "api", "OTF", "graph_items" })
	public void CreateGraphItemsInvalidData() throws JSONException {
		
		String token = MVPApi.signUp(MVPApi.generateUniqueEmail(), "qqqqqq").token;
		
		// use string for average value field
		JSONObject json = DefaultValues.RandomGraphItem().toJson();
		json.put("averageValue", "should be a number");
		...
		BaseParams params = new BaseParams();
		params.addHeader("auth_token", token);
		params.addParam("graph_items", arr.toString());
		...
		params = new BaseParams();
		params.addHeader("auth_token", token);
		params.addParam("graph_item", json.toString());
