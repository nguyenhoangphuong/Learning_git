...
public class OpenApiCaloriesTest {

	static private String accessToken;
	static private String ClientKey;
	static String myEmail = "thinh@misfit.com";
	static String myPassword = "misfit1";
	static String yourPassword = "misfit1";
	static String yourEmail=  "chuotmusic@yahoo.com";
	static String strangerEmail;
    
	static String myToken;
	static String yourToken;
	static String strangerToken;
    
	static String myUid;
	static String yourUid;
	static String strangerUid;
	static OpenAPIThirdPartyApp ClientApp;
	
	private List<Goal> goals;
	
	private String fromDate = MVPCommon.getDateString(System.currentTimeMillis() / 1000 - 3600 * 24 * 1);
	...
	public void GetSummaryWithoutPermission() {
		...
		String invalidScopeAccessToken = OpenAPI.getAccessToken(myEmail, "qqqqqq", OpenAPI.RESOURCE_PROFILE, ClientKey, "https://www.localhost.com.vn/");
	...
	public void GetSummaryUsingAppCredential() {
		...
		// authorized user
		BaseResult result = OpenAPI.getSummary(ClientApp, myUid, toDate, toDate, true);
		OpenAPISummary summary = OpenAPISummary.getSummary(result.response);
		
		Assert.assertEquals(result.statusCode, 200, "Status code");
		//Assert.assertEquals(summary.getCalories(), MVPCommon.round(goals.get(0).getProgressData().getCalorie(), 1), "Calories");
		
		System.out.println("LOG [OpenApiCaloriesTest.GetSummaryUsingAppCredential]: ======= " + summary.getCalories());
		Assert.assertEquals(summary.getDistance(), MVPCommon.round(goals.get(0).getProgressData().getDistanceMiles(), 1), "Distance");
		Assert.assertEquals(summary.getPoints(), goals.get(0).getProgressData().getPoints() / 2.5, "Points");
		Assert.assertEquals(summary.getSteps(), goals.get(0).getProgressData().getSteps(), "Steps");
	...
	public static void main(String[] args) {
        ...
        System.out.println("LOG [OpenApiCaloriesTest.main]: " + ClientApp.getClientKey());
        ClientApp.setClientSecret(ClientSecret);
	    
	    
//	    accessToken = "NO5wasovOw5SYPv9XAAufaNlPl7KmoWGC8jXskyGizizRzEPeBwLZtBsM9Y4d5D9QA2I0bOuf0AX9qNlgLn0XxeW3zSArh18P60TgFELH10zyeXd9bizhn00iZW23FAYPH0vr94rtcmnmnqbbRkqt1feBZi5txTXQFrwInGqebtH8ava1dvG5JJUkH3wEP89Egtk5MrwP7YH1KjWw66HIfl9H4Bt2vJHB0XBIKAIyYe37OjL7chQtb5W04yO2cI";
	            accessToken = OpenAPI.getAccessToken(myEmail, myPassword, OpenAPI.RESOURCE_SUMMARY, ClientKey, "https://localhost/");
        
        myToken = MVPApi.signIn(myEmail, myPassword).token;
//        yourToken = MVPApi.signIn(yourEmail, myPassword).token;
...
//        yourUid = MVPApi.getUserId(yourToken);
        ...
        OpenApiCaloriesTest test = new OpenApiCaloriesTest();
        
        System.out.println("LOG [OpenApiCaloriesTest.main]: ------------------");
        test.GetSummaryUsingAppCredential();
	}
