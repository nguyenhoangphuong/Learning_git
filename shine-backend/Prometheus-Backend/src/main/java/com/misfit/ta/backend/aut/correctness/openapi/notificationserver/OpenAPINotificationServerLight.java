...
public class OpenAPINotificationServerLight extends BackendAutomation implements NotificationReceivedHandler {
	...
	public void beforeClass() {
	    ...
	    //TODO: map the notifications with a predefined set of notifications and fail the test
				...
				try {
					...
					for (int i = 0; i < jarr.length(); i++) {
						...
						resultLogger.log(String.format("Owner: %s - ObjectId: %s - %s: %s", 
	                            mess.getOwnerId(), mess.getId(), mess.getType(), mess.getAction()));
						resultLogger.log("\n");
...
//				resultLogger.log("\n");
	...
	@Test(groups = { "ios", "Prometheus", "MVPBackend", "openapi", "notificationserver", "openapi_light_smoke_test" })
	public void NotificationServerTestScenerio() {
		...
		logger.info("LOG [OpenAPINotificationServerLight.enclosing_method]: number of expected notifications: " + expectedNotifcations);
		logger.info("LOG [OpenAPINotificationServerLight.enclosing_method]: number of received notifications: " + messages.size());
		
		cleanUpSubcriptions();
		Assert.assertTrue(messages.size() > 5, "expected= " + expectedNotifcations + " while received=" + messages.size());
	}
